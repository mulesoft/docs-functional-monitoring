...
In the example project, the test -- `HelloWorld.dwl` -- is in a folder named `tests`. As explained in the section above about the `bat.yaml` file in this project, the `files` section in the `bat.yaml` file points to the tests in a test suite.
