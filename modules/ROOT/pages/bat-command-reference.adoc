...
* `--organization=<value>`
+
The value is the organization name set in the Anypoint Platform profile. If is not present, the environment variable ANYPOINT_ORG is used, otherwise default organization name from credentials file is used.

* `--organizationId=<value>`
+
Value is the organization ID for the Anypoint Platform profile. If is not present ANYPOINT_ORG_ID env variable is used, otherwise default organization ID from credentials file is used.

* `--password=<value>`
+
Value is the password for the Anypoint Platform user name. If is not present ANYPOINT_PASSWORD env variable is used, otherwise default password from credentials file is used.
...
** If it is not present, ANYPOINT_PROFILE env variable is used, otherwise default profile.
...
+
Create reporters from command line.

* `--stack`
+
Print stack traces in standart output during execution.

* `--status-code=<code>`
+
Sets the HTTP status code used for test templates when using command `bat schedule endpoint`. Default value is 200.

* `--timeout=<seconds>`
+
Timeout for tests in seconds. Optional. If is not present BAT_TESTS_TIMEOUT env variable is used, otherwise default value 3600 secs (1 hour).

* `--token=<token>`
+
Value is the auth token for the Anypoint Platform profile.
