...
[IMPORTANT]
====
Previously, for tests to run, you had to allow incoming HTTP traffic into the private location's environment. If you xref:link[update your private location], you'll no longer need to allow incoming HTTP traffic.
====
...
As updates become available, you'll receive notifications in the *Locations* tab. Follow the prompts to apply the update to all private locations.
