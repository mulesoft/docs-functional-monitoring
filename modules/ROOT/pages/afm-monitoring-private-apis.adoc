...
The tests in monitors are run periodically according to a selected schedule. For a private location to run according to a schedule, a schedule must be associated with it. A private location can be associated with more than one schedule for more than one monitor.
...
As updates become available, you'll receive notifications in the *Locations* modal dialog. Follow the prompts to apply the update to all private locations.
