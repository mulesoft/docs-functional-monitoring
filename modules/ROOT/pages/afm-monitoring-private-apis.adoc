...
[NOTE]
====
You can test private APIs from a public location if you whitelist the IP addresses of that location. You might want to test a private API from a public location if you do not want to use vCores on testing private APIs or if you have no vCores available.

In Runtime Manager, add a new rule to the firewall for Anypoint Virtual Private Cloud. The type must be *https.port*, the source must be *Custom*, and the port range 8081. For the source, use an IP address from this list:
