...
You can add schedules to a monitor on more than one schedule if you need to each schedule to run the monitor from a different location, or if you uploaded a monitor that includes multiple configuration files and you want each schedule to use a different configuration.
...
+
*  If at least one schedule already exists, click the three dots in the entry for a schedule and then select *Add Schedule*.
+
* If no schedule yet exists, click *Add Schedule*.
...
* The section "The `config` Folder" in 
