...
[NOTE]
====
Tests silently fail when they run more than 120 seconds. As a workaround, download the monitor and split the test across multiple monitors via the BAT CLI. Run and time each test to ensure their running times are under 120 seconds, then upload the monitors.
