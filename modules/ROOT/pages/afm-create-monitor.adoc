...
* Ensure that your user ID has the Anypoint Monitoring User permission and that it has been assigned the Exchange Administrators role. An administrator for your organization can grant this permission in Anypoint Access Management. For details, see xref:access-management::roles.adoc[Roles].
...
. Tap the *Step 2 - Select endpoints* tab.
. Populate the fields with the information gathered in "Before you Begin".
+
You can preview the test by tapping *Run now* under *Preview the result of your monitor*.

. Tap the *Step 3 - Set notifications* tab.
...
* Tap *Run* (image:afm-ui-run-button.png[width=1%,height=1%]) to immediately execute a monitor.
...
Return to the homepage by tapping *Back to Monitors*.
