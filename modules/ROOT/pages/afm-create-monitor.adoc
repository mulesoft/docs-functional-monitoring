...
[IMPORTANT]
====
Tests silently fail when they take more than 120 seconds to run. As a workaround, you can split a long-running test into multiple tests. For example, if a single test runs against multiple endpoints and takes longer than 120 to finish, try creating multiple tests that each run against a single endpoint. You can split a test by following these steps:

. Create a new test from the long-running test, as explained xref:afm-edit-test.adoc[here].
. In that new test, remove all but one endpoint.
. Follow the first two steps for every endpoint that you want to test.
. Delete the original test.
. Create a schedule for each test.
====
