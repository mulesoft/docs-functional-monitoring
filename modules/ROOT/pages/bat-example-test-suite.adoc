...
The order in which you list the tests determines the order in which the BAT CLI runs them. In this example, the test `orderFullfillment.dwl` runs before the test `shoppingCart.dwl`:
