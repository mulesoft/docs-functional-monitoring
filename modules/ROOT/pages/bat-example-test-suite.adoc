...
If you plan to run a test suite from a private location and the APIs that you plan to test are deployed on CloudHub, you can use the BAT CLI's integration with Anypoint Secrets Manager to use shared secrets in your tests.
...
In the example test suite, the test -- `HelloWorld.dwl` -- is in a folder named `tests`. As explained in the section above about the `bat.yaml` file in this example, the `files` section in the `bat.yaml` file points to the tests in a test suite.
