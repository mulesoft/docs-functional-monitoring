...
In your tests, you can access shared secrets that are symmetric key types from Anypoini Secrets Manager if, which enables you to securely store sensitive information, such as a password, authentication token, endpoint URL, or webhook URL

[IMPORTANT]
====
Functional Monitoring only works with symmetric key shared secret types. Using other types from Anypoint Secrets Monitor will cause every used shared secret stop being populated.
====
