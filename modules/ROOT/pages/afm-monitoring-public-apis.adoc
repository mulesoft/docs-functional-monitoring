...
Tests are run at fixed intervals that you define in schedules. Each test is associated with a schedule. When you run tests against the endpoints of public APIs, AFM sets limits on the following aspects of scheduling:

* *Number of Schedules*: By default, your organization can run up to five schedules at one time for testing from public locations. If you reach the limit of five schedules and want to run more monitors simultaneously, contact your MuleSoft Customer Success Manager about raising your maximum.

* *Duration of Intervals Between Executions of Tests*: Account types of Employee can schedule intervals up to five minutes, and account type of Developer can schedule intervals up to 15 minutes. You can configure intervals using a cron expression.
