...
[IMPORTANT]
====
Reports are emailed only for tests created in API Functional Monitoring in Anypoint Platform, not for tests created with the BAT CLI locally. However,you can create a test with an email reporter through BAT CLI and then upload it to Anypoint Platform. You can also add the email reporter when you load the test in UI or with the option in command schedule
====
