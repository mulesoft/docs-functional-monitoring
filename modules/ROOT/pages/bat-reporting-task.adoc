...
... In the *Key* field, paste your license as a Base64-encoded string.
... Paste the same Base64-encoded string into the *Confirm Key* field.
...
alias_of_the_shared_secret:: Specify the alias that you used for the secret when you ran the `bat grant` command. The alias must be between carets.
. If you do not want to keep your license key secret, add New Relic as a reporter by adding these indented lines to the reporters section:
+
[source,yaml]
----
 - type: NewRelic
