...
To use the email report it is necessary to specify the email address to which you want to notify, it can be a single address or a list (these addresses can be from any email provider). The notification will occur in case of failure. And it will provide the name of the suite in which the error occurs and a list of the statements. This report can only be used for the tests created in Anypoint Functional Monitoring, not with the CLI.
