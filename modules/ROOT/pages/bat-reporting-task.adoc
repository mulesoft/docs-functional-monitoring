...
=== Find AFM metrics in New Relic

To locate AFM metrics in New Relic:

. In New Relic, click *All capabilities* in the left navigation.
. Click *Data explorer*.
. Click *Metrics*.
. In the search bar, type `successfully-afm-execution`.
. (Optional) To narrow the results, add a filter by agent and type `Api Functional Monitoring`.
. Click the metric name to view the time series and related dimensions.

If you prefer NRQL, you can query metrics directly, for example:

[source,sql]
----
FROM Metric 
SELECT count(*) 
WHERE metricName = 'successfully-afm-execution' 
SINCE 1 hour ago TIMESERIES
----

 Use additional filters (for example, by test suite name) to focus on a specific monitor run or environment.
