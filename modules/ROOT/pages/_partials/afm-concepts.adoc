...
Private APIs are APIs with endpoints that are accessible only within your network inside an Anypoint VPC. To test private endpoints, you must use the internal URLs of the application. These are the internal/VPC IP addresses of the Mule workers, in the form of `mule-worker-internal-myapp.region.cloudhub.io`. The VPC must also have access to the applications' ports. See the xref:cloudhub-1::cloudhub-networking-guide.adoc[CloudHub Networking Guide].
