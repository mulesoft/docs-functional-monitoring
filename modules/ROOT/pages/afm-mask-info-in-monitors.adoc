...
You can mask sensitive information in monitors that you run in a private location. Information that can be masked includes headers, token values, username and password, and body values.
...
. Modify the `main.dwl` file to replace the sensitive information with the client ID and client secret you configured in Secrets Manager. In the following example, the sensitive information in headers are masked. The 'clientId-alias' and 'clientSecret-alias' are aliases for the secrets stored in Secrets Manager. See <<grant-access-to-secret>> to learn how to generate the alias.
+
----
import * from bat::BDD
