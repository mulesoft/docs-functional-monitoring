...
To run a schedule in a public location, ensure that your Anypoint Platform organization has not reached its maximum number of schedules. An indicator is displayed when creating a new monitor if there are no public schedules available.
...
. Find the schedule in the list, tap *More* (image:afm-ui-more-button.png[width=0.75%,height=0.75%]) and select *Modify Schedule*.
...
To modify a schedule from the list of schedules using a particular location:
