= Download a Copy of a Monitor, Upload a Test Suite as a Monitor, or Both
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:page-aliases: afm-download-test.adoc, afm-download-monitor.adoc, afm-upload-monitor.adoc

:imagesdir: ../assets/images

You can download a copy of a monitor and then upload the resulting test suite, or you can upload a test suite that you wrote outside of API Functional Monitoring.

== Downloading a Copy of a Monitor and then Uploading the Resulting Test Suite

When you download a copy of a monitor, the result is a _test suite_. A test suite is a bit different from a monitor. A monitor includes one test and one or more schedules, as well as information about reporters that you want to send you notifications of test failures. A test suite can include one or more tests, as well as information about reporters; it contains no schedules. Therefore, when you download a copy of a monitor, the schedules that you created for that monitor are not included in the resulting test suite.

There are two reasons that you might want to download a copy of a monitor and then upload the resulting test suite back into API Functional Monitoring online:

* You downloaded a copy of a monitor from API Functional Monitoring online, edited it as a test suite, and now wish to upload it to replace the original monitor.
+
Do this to include more than one test file in your monitor. The code view in the online interface for API Functional Monitoring does not allow you to create more than one test file.
+
...
* You downloaded a copy of a monitor from API Functional Monitoring online, edited it as a test suite, deleted the file exchange.json that was included in the downloaded ZIP file, and now want to upload the copy as a new monitor.
+
...
== Uploading a Test Suite That You Wrote Outside of API Functional Monitoring

You can upload a test suite a monitor if you wrote the test suite using Behavior Driven Development (BDD), the syntax for writing blackbox-automated-testing (BAT) tests for processing in DataWeave. When you upload the test suite into API Functional Monitoring online in Anypoint Platform, it becomes a new monitor.

.The process involved in uploading a test suite results in a new monitor.
...
* Decide on the frequency at which you would like the monitor to test the endpoints.
+
The shortest interval is every fifteen minutes. You can also choose to run the monitor hourly, daily, weekly, or monthly. The schedule begins immediately after you create the monitor. You can schedule another, more detailed monitor later for the same test suite, if you prefer. You can also modify the schedule after you create the monitor, if you want to specify any of these options:
...
To download a copy of a monitor:

. In the list of your monitors, find the monitor that you want to download a copy of and select it.
. Click the three dots next to the name of the monitor and select *Download Monitor*.
+
*Result:* A ZIP file is downloaded to your computer. The filename is `<your Anypoint Platform group ID>_<the name of the monitor>.zip`.
